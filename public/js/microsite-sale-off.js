$(document).ready(function() {
    ...
    $('.btn-start').on('click', function () {
        ...
        malert('Bạn có muốn kích hoạt trang sale off này không?', 'Xác nhận kích hoạt trang sale off', null, function () {
            ajax_loading(true);

            $.post(_base_url+'/microsite/sale-off/update-status', {id: id, status: 1}, function () {
                ajax_loading(false);
                location.reload();
            });
        });
    ...
    $('.btn-stop').on('click', function () {
        var id = $(this).attr('data-id');
        malert('Bạn có muốn dừng trang sale off này không?', 'Xác nhận dừng trang sale off', null, function () {
            ajax_loading(true);

            $.post(_base_url+'/microsite/sale-off/update-status', {id: id, status: 0}, function () {
                ajax_loading(false);
                location.reload();
            });
        });
            ...
            $.post(_base_url + '/promotion/get-list-products-by-ids', {
                ...
                $.each(item.floors_products, function (index, other) {
                    if (other) {
                        ...
                        add_giff_product(index, other.name, other.class || '', other.ordering, other.sort, product_ids);
                    }
                });
            });
